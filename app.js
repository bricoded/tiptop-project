...
const video = document.querySelectorAll('video.main');


for (let i = 0; i < mute.length; i++) {
    const muteIcon = mute[i];
    muteIcon.addEventListener('click', function () {
        if (!video[i].muted) {
            video[i].muted = true;
        } else if (video[i].muted) {
            video[i].muted = false;
        }
        if (muteIcon.src.indexOf("/tiptop_files/mute.svg") != -1) {
            muteIcon.src = "./tiptop_files/unmute.svg";
        }
        else {
            muteIcon.src = "/tiptop_files/mute.svg";
        }
    })
}